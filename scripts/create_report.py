...
def build_story():
    ...
    story.extend([Spacer(1, 4 * mm), Paragraph("Coordinate handling", styles["H2Custom"]), Paragraph("Input coordinates are assumed to be standard WGS84 longitude/latitude as normally stored in KML. The analysis temporarily uses a local equirectangular approximation centered on the input extent so cell areas and flow distances are measured in metres. Returned geometries are converted back to WGS84 GeoJSON for direct map rendering.", styles["BodyCustom"]), Paragraph("Design rationale", styles["H2Custom"]), Paragraph("A D8 flow model is transparent, reproducible, and extensible for this phase. It gives the frontend a meaningful catchment boundary while keeping later additions independent: rainfall can become a runoff-weighting layer, land ownership can become a constraint mask, and detailed hydrology can replace or refine the routing engine.", styles["BodyCustom"]), Paragraph("Core formulas", styles["H2Custom"]), Paragraph("These formulas are the equations implemented by the terrain engine. R = 6,371,000 m; CI is the contour interval; dx and dy are grid spacings.", styles["BodyCustom"]), table([[P("Component", styles["TableHead"]), P("Formula / rule", styles["TableHead"])], [P("Local coordinates", styles["TableBody"]), P("x = R cos(phi0) (lambda - lambda0)<br/>y = R (phi - phi0)", styles["TableBody"])], [P("Cell area", styles["TableBody"]), P("A_cell = abs(dx * dy)", styles["TableBody"])], [P("D8 flow", styles["TableBody"]), P("f(i) = argmin z(n), for lower n in the eight-cell neighbourhood", styles["TableBody"])], [P("Accumulation", styles["TableBody"]), P("A(i) = A_cell + sum A(k), for upstream k where f(k) = i", styles["TableBody"])], [P("Catchment area", styles["TableBody"]), P("hectares = area_m2 / 10,000", styles["TableBody"])], [P("Storage screening", styles["TableBody"]), P("depth = clamp(2.5 * CI, 1, 8); storage = 0.75 * pond_area * depth", styles["TableBody"])], [P("Future rainfall", styles["TableBody"]), P("runoff_volume = rainfall_m * catchment_area_m2 * runoff_coefficient", styles["TableBody"])]], [43 * mm, 131 * mm]), Paragraph("Candidate cells must be interior and at or above the 85th percentile of flow accumulation. The storage formula is a screening estimate, not a final hydraulic design.", styles["SmallCustom"]), PageBreak()])
