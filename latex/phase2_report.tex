...
\code{http://10.1.75.53:3233/docs} (private remote host, same network only)\par
\textbf{\url{https://reaching-combine-latest-claire.trycloudflare.com/docs}} (public evaluator URL)\par
\textcolor{muted}{\small Temporary Quick Tunnel; URL changes if the tunnel restarts.}\par
\vspace{2mm}
...
\end{lstlisting}
The remote API is running in tmux session \code{pond-api} on assigned Application 1 port \code{3233}; the Cloudflare connector is running separately in \code{cf-pond}. The verified \href{https://reaching-combine-latest-claire.trycloudflare.com/docs}{public evaluator URL} is live, and the \href{https://reaching-combine-latest-claire.trycloudflare.com/health}{health endpoint} returns status \code{ok}. The evaluator can upload a file through the \href{https://reaching-combine-latest-claire.trycloudflare.com/analyzeContour}{analysis endpoint}. This is a temporary Quick Tunnel: it lasts while the \code{cf-pond} process and remote host remain available, but Cloudflare provides no uptime guarantee and the hostname changes after restart. SSH credentials and tokens must remain outside the repository.
