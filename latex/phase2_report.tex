...
\textbf{\code{http://10.1.75.53:3233/docs}} (IIT Bhilai local-network evaluator URL)\par
\textcolor{muted}{\small Assigned Application 1 port derived from SSH port 2233.}\par
\vspace{2mm}
...
Upload contour map & Multipart upload field \textbf{contour\_map}; supports \code{.kml} and \code{.kmz}. & Complete\\
...
Field: contour_map=<contour-map.kml|contour-map.kmz>
\end{lstlisting}
...
\end{lstlisting}
The remote API is running in tmux session \code{pond-api} on Application 1 base port \code{3000}; the IIT system exposes it as external port \code{3233}, derived from SSH port \code{2233}. The verified local-network evaluator URL is \code{http://10.1.75.53:3233/docs}; the \href{http://10.1.75.53:3233/health}{health endpoint} returns status \code{ok}, and the \href{http://10.1.75.53:3233/analyzeContour}{file-upload endpoint} accepts the contour map. These URLs are reachable when the evaluator is connected to the IIT Bhilai local network and the \code{pond-api} session remains running. SSH credentials and tokens must remain outside the repository.
