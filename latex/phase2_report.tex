...
\textbf{\url{https://rate-unable-zope-gcc.trycloudflare.com/docs}} (public evaluator URL)\par
...
\end{lstlisting}
The remote API is running in tmux session \code{pond-api}; the Cloudflare connector is running separately in \code{cf-pond2}. The verified public evaluator URL is \url{https://rate-unable-zope-gcc.trycloudflare.com/docs}, and \url{https://rate-unable-zope-gcc.trycloudflare.com/health} returns status \code{ok}. The evaluator can upload a file through \url{https://rate-unable-zope-gcc.trycloudflare.com/analyzeContour}. This is a temporary Quick Tunnel: it lasts while the \code{cf-pond2} process and remote host remain available, but Cloudflare provides no uptime guarantee and the hostname changes after restart. SSH credentials and tokens must remain outside the repository.
